...
$thisform= load_fwobject("form",$module,4);
...
if (isset($_GET[id]) && $_POST[form_id]!= $thisform["name"])
{
	...
	$PAGE[PAGE_INTITLE]=ADMIN_TA_UPD;
	$PAGE[TXT_TITLE]=ADMIN_TA_UPD;
	$rs=$DB->Execute("SELECT * FROM INT_tariffe WHERE id=".$_GET[id]);
	$result=$rs->FetchRow();
	...
	$response[text]= ADMIN_TA_UPD_DONE_TXT."<br><br>".make_button("ta_view.php",ADMIN_TA_BACK_LIST);
...
{ 
	...
	$PAGE[PAGE_INTITLE]=ADMIN_TA_ADD;
	$PAGE[TXT_TITLE]=ADMIN_TA_ADD;
	$response[title]=ADMIN_TA_ADD_DONE;
	$response[text]= ADMIN_TA_ADD_DONE_TXT."<br><br>".make_button("ta_view.php",ADMIN_TA_BACK_LIST);
