...
$PAGE[TXT_TITLE]=EXPORT_MENU_0_6;
$PAGE[PAGE_INTITLE]=EXPORT_MENU_0_6;
	...
	if ($_GET[form_id]==$thissearch[form][name]){
		...
		if ($error == 1)
		{
			...
			if ($codice_to)  $wh[] = "(codice <= ".$DB->Quote($codice_to).")";
                        if ($descr)      $wh[] = "(nome LIKE ".$DB->Quote("%$descr%").")";
				...
				while ($l = $q->FetchRow()) $GRP[$l[userid]][] = $l[name];
