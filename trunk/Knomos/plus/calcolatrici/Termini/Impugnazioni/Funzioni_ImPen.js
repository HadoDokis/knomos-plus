...
//se sono stati eseguiti i calcoli
	//prende la data di scadenza del termine per il deposito
	CK=document.getElementById("ckDep");
	if(CK.checked==true){vSeTerDep=1}else{vSeTerDep=0}
	txDataScad1D=document.kn2.face.value
	txDataScadD=new Date(txDataScad1D)
	var ggD = txDataScadD.getDate();
	var mmD = txDataScadD.getMonth()+1;
	var aaD =txDataScadD.getFullYear(); 
	//var vTesto = "Richiedere copie sentenza";
	//var vTipoScad = "TRIB";
	//var cod1="XIP20";


	//prende i dati della scadenza dell'impugnazione
...
//alert(vEvento)
	  ...
	  vTipoScad + "&PagProv=" + "CalcImPen"+"&SeTerDep="+vSeTerDep+"&ggD="+ggD+"&mmD="+mmD+"&aaD="+aaD+"&InNota="+vTxInNota
...
//alert(url)
	  ...
	  "&evento=" + vEvento + "&codice=" + cod1 + "&descrizione=" + vTesto + "&priorita=" + vPriorita + "&genere=" +
	  vTipoScad + "&PagProv=" + "CalcImpCalcDeriv"+"&SeTerDep="+vSeTerDep+"&ggD="+ggD+"&mmD="+mmD+"&aaD="+aaD+"&InNota="+vTxInNota
	  parent.location.href=TxUrl
      break
