...
//alert(index)
		...
		{  
			...
			ApriImpegniDerivati() //dalla pag appunt_show (Termine processuale derivato)
		      break;
			default:
			if (index==1){location.href="/modules/calendar/pages/new_app.php?ref_id="+valore;}//NUOVO IMPEGNO (DA TUTTE LE PAGG)
...
function NuovoImpegnoDerivato(valore)
{
		...
		//alert(ddd); alert(mmd);alert(yyd);
	      ...
	      //COMPONE LA URL
	      TxUrl = "/modules/calendar/pages/new_app.php?ref_id=" + valore + "&gg=" + ddd + "&mm=" + mmd + "&aa=" + yyd + "&orario=" + "" +
...
function SelectImpegniAltri111(valore) 
{
//alert(valore)
		switch (valore) 
		{  
...
function StampaImpegno1(descrizione,pratica)
{
alert("qui")
	url="https://www.google.com/calendar/hosted/ammannati.it/render?tab=mc";
	var stile = "top=10, left=10, width=950, height=500, status=no, menubar=no, toolbar=no scrollbar=no";  
	pippo=window.open(url, "", stile);
	el=pippo.getElementById("title");
	el="scrivi"
}
...
{
loadLayerWindow('/plus/calcolatrici/Termini/Processuali/TerminiProcessualiHome1.php?modalita=CalcImpDeriv','Nuovo impegno derivato');
}
	...
	for (i = 0; i < arrCaselleInput.length; i++)
	{
		...
		{  
			...
			arrCaselleInput[i].value = vCod
		      break;
		    //case "done"://fattO
			//if (vFatto == 1) {arrCaselleInput[i].checked=true;} else {arrCaselleInput[i].checked=false;} 
			//arrCaselleInput[i].value=vFatto

		      //break;
		    //case "type"://EVENTO
			//if (vEvento == 0) {arrCaselleInput[i].checked=true;} else {arrCaselleInput[i].checked=false;} 
...
//alert(storico)
	...
	var splittata = vOra.split(":");
		...
		var convPrest = document.FormRinvio.convPrest.checked;
	//e le passa alla pagina php
	FileModificaApp="/modules/calendar/pages/new_app.php?id="+valoreId  + "&gg=" + gg + "&mm=" + mm + "&aa=" + aa + "&hh=" + hh + "&min=" + min + "&exRel=" + exRel + "&PagProv=UltimaUdienza"+ "&convPrest=" + convPrest;
...
//alert(FileModificaApp)
	...
	// distinguendo lo storico impegni
		if(storico=0)
		{
		FileModificaApp="/modules/calendar/pages/new_app.php?id="+valoreId + "&gg=" + gg + "&mm=" + mm + "&aa=" + aa + "&hh=" + hh + "&min=" + min + "&per=" + per + "&ist=" + ist + "&exRel=" + exRel + "&PagProv=Rinvio" + "&convPrest=" + convPrest;
		}
		else
		{
		FileModificaApp="/modules/calendar/pages/new_app.php?ref_id="+vId+"&id_prov="+valoreId +"&gg=" + gg + "&mm=" + mm + "&aa=" + aa + "&hh=" + hh + "&min=" + min + "&per=" + per + "&ist=" + ist + "&exgg=" + exgg + "&exmm=" + exmm+ "&exaa=" + exaa+ "&exCodice="+vCodice+"&exDescrizione="+vDescrizione+"&exRel=" + exRel + "&PagProv=Rinvio" + "&convPrest=" + convPrest;
		}
	parent.location.href=FileModificaApp;
	}
